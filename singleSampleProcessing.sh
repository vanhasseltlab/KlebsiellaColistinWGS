...
Usage:  bash singleSampleProcessing.sh [<R1>.fastq.gz] [<R2>.fastq.gz] [PATH_TO_REFERENCE_GENOME.fa] [PATH_TO_R1-R2_READS] [OUTPUT_DIRECTORY]

Required parameters:
  [<R1>.fastq.gz]               :Name to forward read of sample (fastq.gz format)
  [<R2>.fastq.gz]               :Name to reverse read of sample (fastq.gz format)
  [PATH_TO_REFERENCE_GENOME.fa] :Path to reference genome (fasta format)
  [PATH_TO_R1-R2_READS]         :Path to directory containing R1 and R2 fastq.gz files of sample
...
#===========================================================================================================#
#                           singleSampleProcessing function
singleSampleProcessing () {
  local R1=$1;local R2=$2; local ref=$3; local reads=$4; local outputdir=$5
  local sample=$(echo ${R1} | cut -d_ -f1,2)
  local outdir=${outputdir}/aligned_bwa/${sample}
  mkdir -p ${outdir}
